...
- 點按翻頁可選左右區域或上下區域，並跟隨單本排版設定保存；左右滑動維持翻頁。第一次開書與切換點按方向時，畫面會浮出上一頁／選單／下一頁的區域提示並自動淡出，也可點閱讀選單底部的提示文字再看一次；Aa 面板另有常駐的區域示意圖。左右滑動時頁面跟著手指移動，放開超過螢幕寬度兩成（至少 56px）才翻頁，未達門檻或已在首尾則彈回原位；點按翻頁則以滑動位移呈現（依點按方向決定橫向或縱向）。兩者都可在 Aa 面板「翻頁效果」關閉，系統設為減少動態時自動停用。
...
系列整理：在「批次編輯」選取書籍後，點「系列」填入系列名稱及各本集數；也可從單本書籍資訊編輯。書架將同系列收成一張卡片，點入依集數排列（未填集數排最後），也可在系列頁切換排序。搜尋包含系列名稱，篩選後只顯示及選取符合的集數。系列留白會移出系列，保留原有分類、檔案及閱讀進度。
