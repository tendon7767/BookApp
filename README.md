...
- 點按翻頁可選左右區域或上下區域，並跟隨單本排版設定保存；左右滑動維持翻頁。第一次開書與切換點按方向時，畫面會浮出上一頁／選單／下一頁的區域提示並自動淡出，也可點閱讀選單底部的提示文字再看一次；Aa 面板另有常駐的區域示意圖。翻頁採用滑動位移效果（依點按方向決定橫向或縱向），可在 Aa 面板「翻頁效果」關閉，系統設為減少動態時自動停用。
